...
1.  Bring up a Kafka cluster and start KSQL.
  ...
  * [Follow these instructions if you are using Docker](quickstart-docker.md#docker-setup-for-ksql)  (we recommend Docker for simplicity)
...
![image](ksql-quickstart-schemas.jpg)
    
Before proceeding, please check:

* In the terminal window where you started KSQL, you see the `ksql>` prompt
* If you are not using Docker, you must manually have run the data generator to produce topics called `pageviews` and `users`. If you haven't done this, please follow these [instructions](quickstart-non-docker.md#produce-topic-data) to generate data. (Docker compose file automatically runs the data generator)
